...
class DataSet:
    def __init__(self, N, batch_size):
        ...
        self.x_train, self.F_train, self.U_train, self.S_train, self.S_train_real, self.M_train,\
            self.F_test, self.U_test, self.S_test, self.S_test_real, self.M_test,\
            self.u_train_mean, self.u_train_std, self.f_train_mean, self.f_train_std, self.s_train_mean, \
                self.s_train_std, self.m_train_mean, self.m_train_std = self.samples()
    ...
    def samples(self):
        ...
        S_train_real = screen_all[:num_train, :]
        S_test_real = screen_all[-num_test:, :]
        ...
        return x_train, F_train, U_train, S_train, S_train_real, M_train, F_test, U_test, S_test, S_test_real, M_test, \
            u_train_mean, u_train_std, f_train_mean, f_train_std, s_train_mean, s_train_std, m_train_mean, m_train_std
    ...
    def minibatch(self):    ## random select one training data from f & u
        ...
        return self.x_train, f_train, u_train, s_train, s_train_real, m_train
    ...
    def testbatch(self, num_test):
        ...
        return batch_id, self.x_train, f_test, u_test, s_test, s_test_real, m_test
